...
def get_module_objs(module):
    ...
    s = {n for n, o in inspect.getmembers(module, lambda m: inspect.getmodule(m) is module)}

    # dirty workaround until six fixes metaclass stuff to support EnumMeta in Python >= 3.6
    if module is importlib.import_module('pgpy.types'):
        s.add('FlagEnum')
